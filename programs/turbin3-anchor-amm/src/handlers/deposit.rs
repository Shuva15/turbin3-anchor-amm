...
impl<'info> Deposit<'info> {
    pub fn deposit(
        ...
        let (x, y) = match self.token_mint_lp.supply == 0 && self.token_x_vault.amount == 0 && self.token_y_vault.amount == 0 {
            true => (max_x, max_y),
            false => {
    ...
    pub fn deposit_token(&self, is_x: bool, amount:u64) -> Result<()> {
        let (from, to) = match is_x {
            true => (self.user_token_x.to_account_info(), self.token_x_vault.to_account_info()),
            false => (self.user_token_y.to_account_info(), self.token_y_vault.to_account_info())
        };
